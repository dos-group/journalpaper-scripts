...
startTS=`date +%s`
${OZN_PACT_BIN}/pact-client.sh info -p -j $JOB_STRING > $plnFile 2> $plnFile
${OZN_PACT_BIN}/pact-client.sh run -w -j $JOB_STRING > $outFile 2> $errFile
...
cp ${OZN_PACT_LOG}/* ${EXP_LOG_DIR}/${EXP_ID}/stratosphere-logs
