...
./jp_load_data_ts.sh ${TS_SCALING_FACTOR} ${TS_NODE_COUNT} ${TS_DGEN_ID}
if [[ $? != 0 ]]; then
   exit $?
fi
...
./jp_run_repeated.sh HDP_MAPR $execIdPrefix "${EXP_JOBS_HOME}/journalpaper-jobs-1.0.0-ts_hdfs-hdp_mapr.jar ${TS_NODE_COUNT} ${TS_IN} ${TS_OUT}"

# repeat Stratosphere runs
execIdPrefix=`printf "ts-str_pact-sf%04d-dop%04d" ${TS_SCALING_FACTOR} ${TS_NODE_COUNT}`
./jp_run_repeated.sh STR_PACT $execIdPrefix "${EXP_JOBS_HOME}/journalpaper-jobs-1.0.0-ts_hdfs-str_pact.jar -a ${TS_NODE_COUNT} ${TS_IN} ${TS_OUT}"
