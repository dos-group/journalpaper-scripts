...
# Parameters:
# 1) Execution System: System to run the job on (HMR, HON, STR)
...
if [[ $EXEC_SYSTEM != 'HMR'  && $EXEC_SYSTEM != 'STR' ]]
then
  echo "You need to specify an system to execute (HMR, STR). Canceling..."
  exit 1
fi
